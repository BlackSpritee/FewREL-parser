...
class FewRelDataset(data.Dataset):
    ...
    def __getraw__(self, item):
        word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask = self.encoder.tokenize_parserAndRawAddClsAvg(item['tokens'],
                                                                                 item['h'],
                                                                                 item['t'],
                                                                                 item['parser'])
        return word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask

    def __additem__(self, d, word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask):
        d['word'].append(word)
        d['pos1'].append(pos1)
        d['pos2'].append(pos2)
        d['mask'].append(mask)
    ...
    def __getitem__(self, index):
        ...
        support_set = {'word': [], 'pos1': [], 'pos2': [], 'mask': [], 'lens':[], 'pos1_end':[], 'pos2_end': [],'parser_text':[], 'parser_mask':[]}
        query_set = {'word': [], 'pos1': [], 'pos2': [], 'mask': [], 'lens':[], 'pos1_end': [], 'pos2_end': [],'parser_text':[], 'parser_mask':[] }
        query_label = []
        ...
        for i, class_name in enumerate(target_classes):
            ...
            count = 0
            for j in indices:
                word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask = self.__getraw__(
                ...
                if count < self.K:
                    self.__additem__(support_set, word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask)
                else:
                    self.__additem__(query_set, word, pos1, pos2, mask, lens, pos1_end, pos2_end,parser_text,parser_mask)
                count += 1
        ...
        for j in range(Q_na):
            ...
            index = np.random.choice(
                ...
                1, False)[0]
            word, pos1, pos2, mask, pos1_end, pos2_end,parser_text,parser_mask = self.__getraw__(
            ...
            parser_text=torch.tensor(parser_text).long()
            parser_mask=torch.tensor(parser_mask).long()
            self.__additem__(query_set, word, pos1, pos2, mask, pos1_end, pos2_end,parser_text,parser_mask)
        # NA
...
def collate_fn(data):
    batch_support = {'word': [], 'pos1': [], 'pos2': [], 'mask': [], 'lens': [], 'pos1_end':[], 'pos2_end': [] ,'parser_text':[], 'parser_mask':[]}
    batch_query = {'word': [], 'pos1': [], 'pos2': [], 'mask': [], 'lens': [], 'pos1_end': [], 'pos2_end': [] ,'parser_text':[], 'parser_mask':[]}
