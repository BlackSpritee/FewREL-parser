...
class BERTSentenceEncoder(nn.Module):
    ...
    def forward(self, inputs, cat=True):
        if not self.cat_entity_rep:
            ...
            outputs_raw = self.bert(inputs['word'], attention_mask=inputs['mask'])
            if cat:
                ...
                h_state = outputs_raw['last_hidden_state'][tensor_range, inputs["pos1"]] # h_state.shape [20, 768]
                t_state = outputs_raw['last_hidden_state'][tensor_range, inputs["pos2"]] # [20, 768]
                ...
                h_state=parser_cls+h_state
                t_state=parser_avg+t_state
                ...
                return h_state, t_state, outputs_raw['last_hidden_state'] #,parser_cls,parser_avg
            else:
                
                return outputs_raw['pooler_output'], outputs_raw['last_hidden_state']
    
    def tokenize(self, raw_tokens, pos_head, pos_tail, parser_tokens):
        # token -> index
        tokens = ['[CLS]']
        cur_pos = 1
        pos1_in_index = 1
    ...
    def tokenize_parserAndRaw(self, raw_tokens, head,tail, parser_tokens):
        ...
        raw_tokens.extend(parser_tokens)
    ...
    def tokenize_parserAndRawAddClsAvg(self, raw_tokens, head,tail, parser_tokens):
        ...
        for token in parser_tokens:
            if  cur_pos == 1:
                parser_tokensed.append('[unused0]')
                pos1_in_index=len(tokens)
                parser_tokensed += self.tokenizer.tokenize(token.lower())
                pos1_end_index=len(tokens)
                parser_tokensed.append('[unused2]')
            elif cur_pos == len(parser_tokens):
                parser_tokensed.append('[unused1]')
                pos2_in_index=len(tokens)
                parser_tokensed += self.tokenizer.tokenize(token.lower())
                pos2_end_index=len(tokens)
                parser_tokensed.append('[unused3]')
            else:
                parser_tokensed += self.tokenizer.tokenize(token.lower())
            cur_pos+=1
        parser_indexed_tokens = self.tokenizer.convert_tokens_to_ids(tokens)
