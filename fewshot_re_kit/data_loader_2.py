...
class FewRel2Dataset(data.Dataset):
    """
    FewRel Dataset
    """
    def __init__(self, name, encoder, N, K, Q, na_rate, root):
    ...
    def __getrel2__(self, item):
        word, mask = self.encoder.tokenize_rel2(item)
        return word, mask
    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            ...
            #TODO
            rel_text, rel_text_mask = self.__getrel2__(class_name)
            rel_text, rel_text_mask = torch.tensor(rel_text).long(), torch.tensor(rel_text_mask).long()
            relation_set['word'].append(rel_text)
            relation_set['mask'].append(rel_text_mask)
...
def get_loaderfewrel2(name, encoder, N, K, Q, batch_size,
               num_workers=8, collate_fn=collate_fn, na_rate=0, root='./data'):
    dataset = FewRel2Dataset(name, encoder, N, K, Q, na_rate, root)
    ##TODO
    #import pdb
    #pdb.set_trace()
